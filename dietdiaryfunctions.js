...
function buildDaysDataTable (data) {
    ...
    strhtml += "<thead><tr><th width=25>Name</th><th>Amount</th><th>Portion</th><th>Calories</th><th>Carbs</th><th width='10px'> X </th></tr></thead>";
    strhtml += "<tbody>";
    if (data[0] !== undefined) {
        for (var i in data) {
            thisval = data[i]['Calories']*data[i]['Amount'];
            totalval += thisval;
            thiscarbs = data[i]['Carbs']*data[i]['Amount'];
            ...
            strhtml = strhtml + "<td align='right'>" + thiscarbs.toFixed(2) + "</td>";
    ...
    strhtml += "<td id='foodcarbscell'></td>";
    strhtml += "<td id='foodimageplus' class='foodimageplus' onclick='addDayFoodEntry()' title='Add...'><img src='./_images/open.png'></td>";
    strhtml += "</tr>";
    // and now the totals
    strhtml += ("<tr><td></td><td></td><td>Totals : </td><td>" + totalval.toFixed(2) + "</td><td>" + totalcarbs.toFixed(2) + "</td><td></td></tr>");
    strhtml += "</tbody>";
    strhtml += "</table>"; 
        ...
        select: function(event, ui) {
            ...
            $('#foodamountinput').blur(function () {
                if ($('#foodamountinput').val()==="") {
                    ...
                    $('#foodcarbscell').val($('#foodcarbscell').val()*$('#foodamountinput').val());
                    $('#foodcarbscell').text($('#foodcarbscell').val());
                    addDayFoodEntry();
                }
            }); 
        }
...
function getRecipeppCarbs (recipeid) {
    $.get('getRecipeppCalories.php',
        { recipeid : recipeid },
        function(data) {
            $('#foodportionname').text(data[0]['foodportionname']).val(data[0]['foodportionname']);
            $('#foodcarbscell').text(data[0]['ppCarbs']).val(data[0]['ppCarbs']);
            return data[0]['ppCarbs'];
        },
        'json'
    );
}
...
function addIngredient(ingredientfoodcode) {
    //alert ("function addIngredient() " + ingredientfoodcode + " recdetail =" + $('#recdetail').val())
    if ($('#recdetail').val() === "" || $('#recdetail').val()==="NoRecipe") {   // if this is a brand new recipe
      // we first have to create the recipe!!
      //alert ("trying to add " + $('#recipeName').val() )
            ...
            function() {
                ...
                $.post('getRecipeFoodCodeByName.php',
                    ...
                    function(data) {
                        ...
                        //alert ('275 recdetail= '+$('#recdetail').val()+' recipeid2= '+$('#recipeid2').val());
                        addIngredientToRecipe(ingredientfoodcode);
                    },
                'json'
                );
                //alert ("after post getRecipeFoodCodeByName: recipename = " + $('#recipeName').val() )
            }
        );
...
function addIngredientToRecipe(ingredientfoodcode) {
    if ($('#recdetail').val() === "" || $('#recdetail').val() === "NoRecipe") {
        alert (" addIngredientToRecipe Line 286 NO RECIPE DETAILS AVAILABLE - can't add ingredient");
        $('#recipeName').focus();
    } else {
        ...
        $.post('getRecipeIngredients.php',
            ...
            function(data, status) {
                ...
                for (var i in data) {
                    ...
                    carbtotal += data[i]['ppCarbs']/data[0]['How_Many'];   // calculate the ppvalues
                    strhtml += "<tr>";
                    ...
                    strhtml += "<td align='right'>" + data[i]['ppCarbs'] + "</td>";
...
function setStandardPrequel() {
    ...
    str += "<thead><tr><th width='230px'>Name</th><th>Amount</th><th>Portion</th><th>Calories</th><th>Carbs</th><th width='10px'> X </th></tr></thead>";
    str += "<tbody>";        
    return str;
}
function setStandardSequel() {
    ...
    // and now the totals
    str += ("<tr><td></td><td></td><td>Totals (p.p.) : </td><td align='right'>" + recipetotal.toFixed(2) + "</td><td align='right'>" + carbtotal.toFixed(2) + "</td><td></td></tr>");
    str += "</tr>";
    str += "</tbody>";
    str += "</table>"; 
...
function finaliseTable() {
    ...
    $('#tableinput').autocomplete({
        ...
        select: function(event, ui) {
            ...
            $('#blankcell2').html(ui.item.carbs + "pp");
    ...
    $('#newamountcell').keypress(function(e) {
        if(e.which === 9) {
            ...
            carbval = 0.0;
            carbval = ui.item.carbs*$('#newamountcell').val()/$('#ingredientportionname').val();
            carbval = carbval.toFixed(2);              
            $('#blankcell2').html(carbval + "pp");
    ...
    $('#newamountcell').keypress(function(e) {
        if(e.which === 13) {
            $('#currentingredientrow td:eq(5)').click(); // this clicks the add button
        }
    });
...
function addNewIngredient () {
    ...
    if ($('#food_name').val() !== "") {
        $.post('addPortionRecipe.php', 
            ...
            barcode : ($('#barcode').val()==="","00000000000000",$('#barcode').val()),
