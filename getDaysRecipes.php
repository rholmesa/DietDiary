...
$sql = "SELECT did as id, food_code as recipeFoodCode, amount as Amount FROM diary where entrydate = '".$mydate."' AND userid='".$userid."' ORDER BY did;";
...
if ($entries) {
    while ($row = $entries->fetch_assoc()) {
        ...
        if ($result) {
            ...
            $a_json_row["Portion Carbs"] = $data['carbohydrate_g'];
            $a_json_row["Calories"] = $data['ppCalories'];
            $a_json_row["Carbs"] = $data['ppCarbs'];
